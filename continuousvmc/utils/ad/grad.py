...
def value_and_grad(
    ...
    def value_and_grad_f(*args, **kwargs):
        ...
        if tree_is_complex([args[i] for i in _args_iterable]):

            if is_complex(out_shape):  # C -> C

                return jax.value_and_grad(
                    fun, argnums=argnums, has_aux=has_aux, allow_int=allow_int, holomorphic=True
                )(*args, **kwargs)
                ...
                def grad_rc(*args, **kwargs):
                    ...
                    if has_aux:
                        ...
                        out_r, grad_r = jax.value_and_grad(
                            real_fun, argnums=argnums, has_aux=False, allow_int=allow_int
                        )(*args, **kwargs)
                    ...
                    out = out_r + 1j * out_j
                    grad = tree_map(lambda re, im: re + 1j * im, grad_r, grad_j)
